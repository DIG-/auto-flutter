...
class BuildConfigFlavored(BuildConfig):
    flavored: Optional[Dict[str, BuildConfig]]

    def get_build_param(self, flavor: Optional[str]) -> str:
        output = ""
    ...
    def get_run_before(self, flavor: Optional[str]) -> Dict[BuildRunBefore, List[str]]:
    ...
    def get_output(self, flavor: Optional[str], type: BuildType) -> Optional[str]:
        if not flavor is None and flavor in self.flavored:
