...
from ..model.argument.option.opt_all_codec import ArgsEncode
    ...
    def execute(self, args: Args) -> TaskResult:
        ...
        skip = (
            not _Iterable.first_or_none(
                options, lambda x: isinstance(x, OptionAll))
            is None
        )

        if skip:
            encoder = ArgsEncode(args)
        ...
        for opt, value in opts:
            ...
            if found is None:
                ...
                raise TypeError(
                    "Unexpected instance of Option: {}".format(
                        type(found).__name__)
                )
